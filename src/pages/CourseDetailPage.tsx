    ...
    <div className="min-h-screen bg-transparent text-gray-100 font-sans selection:bg-blue-500/30">
      ...
      <main className="pt-32 pb-20 px-6">
        <div className="max-w-7xl mx-auto">
          {/* Back Link */}
          <Link to="/tudastar" className="inline-flex items-center gap-2 text-gray-500 hover:text-white transition-colors mb-12 group">
            ...
            <div className="lg:col-span-2 space-y-16">
              ...
              >
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <span className="text-gray-500 text-xs font-medium">• {course.level} szint</span>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-8 border-y border-white/5 py-10">
                <div className="text-center md:text-left">
                  <div className="text-gray-500 text-xs font-bold uppercase tracking-widest mb-2 flex items-center justify-center md:justify-start gap-2">
                    <Star className="w-3.5 h-3.5 text-orange-400 fill-current" /> Értékelés
                  </div>
                  <div className="text-2xl font-bold text-white">{course.rating} / 5.0</div>
                </div>
                <div className="text-center md:text-left">
                  <div className="text-gray-500 text-xs font-bold uppercase tracking-widest mb-2 flex items-center justify-center md:justify-start gap-2">
                ...
                <div className="text-center md:text-left">
                  ...
                  <div className="text-2xl font-bold text-white">Elérhető</div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  {course.features.map((feature) => (
                    <div key={feature} className="flex items-start gap-4 p-6 rounded-2xl bg-white/5 border border-white/5">
              ...
              <div>
                ...
                <div className="space-y-4 relative">
                  {course.curriculum?.map((item, index) => {
                    ...
                    return (
                      <div key={item.title} className={`flex items-center justify-between p-6 rounded-2xl bg-[#0d0d0d] border border-white/5 transition-all group ${isLocked ? 'opacity-50 grayscale' : 'hover:border-blue-500/30'}`}>
                        <div className="flex items-center gap-6">
                          <div className="text-gray-600 font-bold text-lg">{String(index + 1).padStart(2, '0')}</div>
                          <div className="flex items-center gap-4">
                            {isLocked ? (
                              <Lock className="text-gray-600 w-5 h-5" />
                            ) : (
                              <PlayCircle className="text-gray-500 group-hover:text-blue-500 transition-colors w-5 h-5" />
                            )}
                            <span className="text-gray-300 font-bold">{item.title}</span>
                          </div>
                        </div>
                        <div className="text-gray-500 text-sm font-medium">{item.duration}</div>
                      </div>
                    );
                  })}
                  ...
                  {course.accessType === 'premium' && !isPremium && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black/20 backdrop-blur-[2px] rounded-3xl">
                      <div className="bg-[#151515] border border-white/10 p-8 rounded-3xl text-center shadow-2xl max-w-sm">
                        ...
                        <h3 className="text-xl font-bold mb-3">Prémium Tartalom</h3>
            ...
            <div className="space-y-8">
              <div className="sticky top-32">
                <motion.div
                  ...
                  className="bg-[#0d0d0d] border border-white/10 rounded-[3rem] p-8 md:p-10 shadow-3xl overflow-hidden relative"
                >
                  {/* Image Preview */}
                  ...
                  <div className="space-y-8">
                    <div className="flex items-end gap-3">
                      ...
                      {course.price !== 'Ingyenes' && !(course.accessType === 'premium' && isPremium) && (
                        <span className="text-gray-500 line-through mb-1">49.900 Ft</span>
                      )}
                      {course.accessType === 'premium' && isPremium && (
                        <span className="text-orange-400 text-xs font-bold uppercase tracking-widest mb-2 flex items-center gap-1">
                    ...
                    <div className="space-y-4 pt-4">
                      <p className="text-xs font-bold text-gray-500 uppercase tracking-widest text-center">A kurzus tartalmazza:</p>
                ...
                {/* Promo Card */}
                <div className="mt-8 p-8 rounded-[2.5rem] bg-gradient-to-br from-blue-600/20 to-purple-600/20 border border-white/10 text-center">
                  <h4 className="text-lg font-bold mb-2">Csoportos kedvezmény?</h4>
                  <p className="text-gray-400 text-sm mb-6">Cégeknek és csapatoknak egyedi árajánlatot biztosítunk.</p>
                  <Link to="/contact" className="text-blue-400 font-bold text-sm hover:underline">Vedd fel velünk a kapcsolatot</Link>
                </div>
              </div>
            </div>
          </div>
